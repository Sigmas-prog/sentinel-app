   ...
   child: Padding(
     ...
     child: CyberCard(
       ...
       child: Column(mainAxisSize: MainAxisSize.min, children: [
         const Icon(Icons.hub, color: SentinelTheme.violet, size: 90),
         const SizedBox(height: 20),
         Text(_status, style: const TextStyle(color: SentinelTheme.violet)),
         const SizedBox(height: 12),
         LinearProgressIndicator(value: _progress, minHeight: 9, color: SentinelTheme.green, backgroundColor: SentinelTheme.background),
         const SizedBox(height: 10),
         Text('${(_progress * 100).round()}%', style: const TextStyle(fontSize: 24, color: SentinelTheme.cyan)),
         const SizedBox(height: 20),
         GlowButton(label: 'ЗАПУСТИТЬ ЭФФЕКТ', color: SentinelTheme.violet, icon: Icons.play_arrow, onPressed: _start),
       ]),
     ),
   ),
 );

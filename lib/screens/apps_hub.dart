...
class AppsHubScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final apps = <_AppItem>[
      ...
      _AppItem('MEDIA', 'Музыкальный плеер', Icons.graphic_eq, SentinelTheme.violet, const MediaScreen()),
      _AppItem('CHAT', 'Локальные заметки', Icons.forum_outlined, SentinelTheme.green, const MessagesScreen()),
      _AppItem('CONTACTS', 'Команда и статусы', Icons.people_outline, SentinelTheme.cyan, const ContactsScreen()),
      _AppItem('PROFILE', 'Личный профиль', Icons.person_outline, Colors.orangeAccent, const ProfileScreen()),
      _AppItem('VISUALS', 'Экран визуализации', Icons.hub_outlined, SentinelTheme.violet, const EffectScreen()),
    ];
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        const Text('APPS // HOME', style: TextStyle(fontSize: 21, color: SentinelTheme.violet, fontWeight: FontWeight.bold)),
        const SizedBox(height: 6),
        const Text('Личный набор инструментов'),
        ...
        const SizedBox(height: 16),
        CyberCard(title: 'SYSTEM', child: const Text('SENTINEL OS // интерфейс персонального телефона', style: TextStyle(fontSize: 12))),
      ],
    );
  }
}

...
class _TerminalScreenState extends State<TerminalScreen> { final _input = TextEditingController(); final _service = TerminalService(SystemInfoService(), WifiScannerService()); final List<String> _lines = ['SENTINEL TERMINAL v1.0', 'Введите help для списка команд.']; bool _busy = false;
 ...
 @override Widget build(BuildContext context) => Column(children: [Expanded(child: Container(margin: const EdgeInsets.all(14), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.black.withOpacity(.72), border: Border.all(color: SentinelTheme.violet)), child: ListView(children: _lines.map((e) => Padding(padding: const EdgeInsets.only(bottom: 8), child: Text(e, style: const TextStyle(fontSize: 12, color: Color(0xFFD6F5FF)))).toList()))), Padding(padding: const EdgeInsets.fromLTRB(14,0,14,16), child: TextField(controller: _input, enabled: !_busy, onSubmitted: (_) => _run(), style: const TextStyle(color: SentinelTheme.green), decoration: InputDecoration(prefixText: '> ', prefixStyle: const TextStyle(color: SentinelTheme.green), hintText: _busy ? 'выполняется...' : 'help', border: const OutlineInputBorder()),))]); }

...
class _ScannerScreenState extends State<ScannerScreen> { final _scanner = WifiScannerService(); Future<List<WifiNetwork>>? _networks; String? _error;
 ...
 Widget build(BuildContext context) => ListView(
   ...
   children: [
     const Text('WI-FI // ОБЗОР', style: TextStyle(fontSize: 21, color: SentinelTheme.green, fontWeight: FontWeight.bold)),
     const SizedBox(height: 7),
     const Text('Локальная визуализация доступных сетей'),
     const SizedBox(height: 14),
     GlowButton(label: 'ЗАПУСТИТЬ СКАНИРОВАНИЕ', color: SentinelTheme.green, icon: Icons.wifi_find, onPressed: _scan),
     const SizedBox(height: 14),
     ...
     if (_networks != null)
       FutureBuilder<List<WifiNetwork>>(
         ...
         builder: (_, snapshot) {
           ...
           return Column(
             children: [
               Text('НАЙДЕНО: ${list.length}', style: const TextStyle(color: SentinelTheme.muted)),
               const SizedBox(height: 10),
               ...list.map((network) => Padding(
                 ...
                 child: CyberCard(
                   ...
                   child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                     ...
                     Text('SIGNAL ${network.level}   CHANNEL ${network.channel}', style: const TextStyle(color: SentinelTheme.green, fontSize: 11)),
                   ]),
                 ),
               )),
             ],
